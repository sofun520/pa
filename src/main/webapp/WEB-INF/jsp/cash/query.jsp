								...
								<c:if test="${list!=null}">
									<c:forEach var="ca" items="${list}" varStatus="xh">
										<tr>
											...
											<td><c:out value="${ca.cashCardId}" /></td>
											<td><c:out value="${ca.cashAmount}" /></td>
											<td><c:out value="${ca.cashFate}" /></td>
											<td><fmt:formatNumber type="number"
													value="${ca.cashAmount*ca.cashFate}" pattern="0.00"
													maxFractionDigits="2" /></td>
											<td><fmt:formatDate value="${ca.cashDate}"
													pattern="yyyy-MM-dd" /></td>
											<td><a href="delete.do?id=${ca.cashId}"><i
					...
					<form class="form-horizontal" id="addForm" method="post"
						...
						<div class="box-body">
							<div class="form-group">
								...
								<div class="col-sm-8">
									<select class="form-control input-sm" name="cashCardId"
										check-type="required">
										<c:forEach items="${cardList}" var="ca">
											<option value="${ca.cardId}">${ca.cardNo}(${ca.cardBank})</option>
										</c:forEach>
									</select>
								</div>
							...
							<div class="form-group">
								<label for="inputEmail3" class="col-sm-2 control-label">费率</label>
								<div class="col-sm-8">
									<input type="text" name="cashFate"
										class="form-control input-sm" id="inputEmail3" value="0.005"
										check-type="required">
								</div>
								<div class="col-sm-2"></div>
							</div>
